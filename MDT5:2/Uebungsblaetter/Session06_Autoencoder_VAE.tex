...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small MDT5/2 -- Session 6: Autoencoder \& Variational Autoencoder}
...
\begin{center}
  ...
  {\large Kapitel 7/7a: Autoencoder, CAE, Ensembles (RandNet), VAE -- Praktika 06 + 07}\\[4pt]
...
\subsection*{1.2 \ Für den Latent Space gilt:}
\mcbox Beim gewöhnlichen Autoencoder wird die Verteilung des Latent Space im Training
explizit auf eine Normalverteilung eingestellt.

\mcbox Beim Variational Autoencoder erzwingt die Kullback-Leibler-Divergenz als
Regularisierer eine im Training festgelegte A-priori-Verteilung des Latent Space.

\mcbox Der Latent Space hat immer eine höhere Dimension als die Eingabedaten.

Begründung:
\antwortlinien{2}

\subsection*{1.3 \ Für das Upsampling im Decoder eines Convolutional Autoencoders gilt:}
...
\subsection*{1.4 \ Für Ensembles von Autoencodern gilt:}
...
\section*{Aufgabe 5 -- Variational Autoencoder}

a) Nennen Sie zwei Limitierungen des gewöhnlichen Autoencoders, die den VAE motivieren.
\antwortlinien{3}

b) Der Encoder eines VAE gibt keine festen Codes aus. Was gibt er stattdessen aus,
und wie entsteht daraus $\boldsymbol{z}$?
\antwortlinien{3}

c) Welche Rolle spielt die Kullback-Leibler-Divergenz in der Kostenfunktion des VAE?
\antwortlinien{2}

d) Erklären Sie den Reparameterisierungs-Trick
($\boldsymbol{z} = \boldsymbol{\mu} + \boldsymbol{\sigma} \odot \boldsymbol{\epsilon}$ mit
$\boldsymbol{\epsilon}\sim\mathcal{N}(0,\boldsymbol{I})$): Welches Problem löst er?
\antwortlinien{3}

e) Wie wird ein trainierter VAE zur Novelty Detection eingesetzt
(Stichwort Reconstruction Probability)? Und wie kann er zur Datengenerierung genutzt werden?
\antwortlinien{4}
