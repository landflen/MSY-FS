...
% ============================================================
\teilkopf{1}{Teil 1 · Grundlagen (Säule A: WDF \& Transformation)}
...
\begin{tabular}{@{}l|l@{}}
  ...
  \multicolumn{2}{@{}p{\linewidth}}{\rule{0pt}{1.1em}%
    \textbf{Quadrat einer Summe} {\scriptsize\itshape(wichtig beim MSE):}\quad
    $(a{+}b{+}c)^2=\underbrace{a^2{+}b^2{+}c^2}_{\text{3 Quadrate}}
      +\underbrace{2ab{+}2ac{+}2bc}_{\text{3 Mischterme}}$\newline
    {\scriptsize\itshape Allgemein: $n$ Terme $\to$ $n$ Quadrate
    $+\binom{n}{2}$ Mischterme; im Erwartungswert $E\{(\cdot)^2\}$ fallen
    Mischterme von unabh., mittelwertfreien Größen weg ($E\{a\,n\}{=}0$).}} \\[0.3em]
...
\bereich[cMath]{Transponierte $A^T$ (Rechenregeln)}
\noindent\footnotesize\setlength{\tabcolsep}{6pt}%
\begin{tabular}{@{}l|l@{}}
  Zeilen $\leftrightarrow$ Spalten: $n{\times}q \to q{\times}n$ &
  $(A^T)^T=A$ \\[0.3em]
  $(A+B)^T=A^T+B^T$ &
  $(cA)^T=c\,A^T$ \\[0.3em]
  $(AB)^T=B^T A^T$\; {\scriptsize\itshape (Reihenfolge dreht!)} &
  Symmetrisch: $A=A^T$ {\scriptsize\itshape (z.\,B.\ Kovarianz)} \\[0.3em]
  \textbf{Skalar:} $a^T=a$ {\scriptsize\itshape (wirkungslos)} &
  $\Rightarrow G\,Z_0\,G^T=G^2 Z_0$ \\[0.3em]
  \multicolumn{2}{@{}l}{\textbf{Beispiel} ($2{\times}3\to3{\times}2$):\quad
    $A=\begin{psmallmatrix}1&2&3\\4&5&6\end{psmallmatrix}
    \;\Rightarrow\;
    A^T=\begin{psmallmatrix}1&4\\2&5\\3&6\end{psmallmatrix}$
    \;{\scriptsize\itshape (1.\ Zeile wird 1.\ Spalte usw.)}} \\
\end{tabular}\par
\bereichende
...
\begin{formelreihe}
  ...
  \formelblock{Satz von Bayes}{\boxed{P(A\mid B)=\dfrac{P(B\mid A)\,P(A)}{P(B)}}} &
...
\begin{formelreihe}
  ...
  \formelblock{Dichte-Transformation}{\boxed{f_Y(y)=\dfrac{f_X(x)}{|y'(x)|}}} &
...
\textbf{Begrenzer / waagrechter Kennlinienast ($g'{=}0$):} ein ganzer
$x$-Bereich wird auf \emph{einen} $y$-Wert $y_c$ abgebildet $\Rightarrow$
\textbf{Dirac} in $f_Y$ bei $y_c$, Gewicht $=$ Wahrscheinlichkeit dieses Bereichs
$w=\int_{\text{Klippbereich}}\! f_X(x)\,\mathrm dx$.\;
Formel $f_Y{=}f_X/|g'|$ versagt dort ($g'{=}0$, Division durch Null) —
Fläche stattdessen als Punktmasse aufsammeln.\;
Schräge Äste ($g'{\neq}0$) wie gewohnt; typische Klausur-Kombination
\emph{Begrenzer} $=$ mittlere Rampe $+$ zwei Diracs an den Sättigungswerten.\par
\miniexample{stückw.\ lineare Kennlinie \;/\; Kennlinie $g(x){=}\sqrt{x}$
  ...
  Begrenzer $y{=}x$ für $|x|{\le}1$, sonst $\pm1$, $X{\sim}\mathcal U[-2,2]$ ($f_X{=}\tfrac14$):\;
  Rampe $f_Y{=}\tfrac14$ auf $(-1,1)$ $+$ Diracs bei $y{=}{\pm}1$ mit Gewicht
  $w{=}P(x{>}1){=}\tfrac14$ bzw.\ $P(x{<}{-}1){=}\tfrac14$
  $\Rightarrow \tfrac14\delta(y{-}1)+\tfrac14\delta(y{+}1)$ (Kontrolle: $\int f_Y=\tfrac12+\tfrac14+\tfrac14=1$).}
\bereichende
