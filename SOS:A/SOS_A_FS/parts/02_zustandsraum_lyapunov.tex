...
\begin{formelreihe}
  ...
  \parbox[t]{\linewidth}{%
    ...
    \scriptsize
    \textbf{Anschaulich:} kleine Anfangsauslenkung $\Rightarrow$ Traj.\ bleibt \emph{nahe} der RL — muss \textbf{nicht} zurückkehren. Grenzfall zw.\ asym.\ stabil \& instabil.\\[0.08em]
...
\noindent\footnotesize\textbf{Betriebspunkt aus Soll-Ausgang $y_B$} (typ.\ Klausurform): Ausgangsgl.\ $y_B=h(\mathbf{x}_B,u_B)$ und alle $\dot x_i=0$ als Gleichungssystem; gegebene Größe einsetzen, übrige schrittweise auflösen (meist: Ausgangsgl.\ $\to x_{jB}$, dann $\dot x_2=0\to$ weitere, dann $\dot x_1=0\to u_B$).\par\vspace{0.1em}
