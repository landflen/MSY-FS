...
\begin{formelreihe}
  ...
  \infoblock{$\mathbf{x}_B$ wird zur Ruhelage\\
    des linearisierten $\Delta$-Systems\\
    (s. Teil\,5:
  ...
  \formelblock{Stabil nach Lyapunov (iSvL)}{%
    ...
    \Rightarrow\|\mathbf{x}(t)-\mathbf{x}_R\|<\varepsilon\;\forall t\geq0} &
  \parbox[t]{\linewidth}{%
...
\begin{formelreihe}
  ...
  \end{aligned}} &
  \infoblock{Terme ohne $\Delta$ fallen\\
    heraus, da im BP gilt:\\
    $f(\mathbf{x}_B,u_B)=\mathbf{0}$\\[0.1em]
    Ergebnis: $\Delta$-System\\
    mit Ruhelage $\Delta\mathbf{x}=\mathbf{0}$\\
    $(\Leftrightarrow\mathbf{x}=\mathbf{x}_B)$\\[0.05em]
    \textup{= Trafo in die Ruhelage}\\[0.07em]
...
\begin{formelreihe}
  ...
  \parbox[t]{\linewidth}{%
    ...
    \end{aligned}}\\[0.25em]
    \infoblock{$A_{B,ij}$: part. Abl. von $f_i$\\
      nach $x_j$, ausgewertet im BP.\\[0.06em]
...
\begin{formelreihe}
  ...
  \parbox[t]{\linewidth}{%
    ...
    \scriptsize
    $\mathbf{Q}$ pos.\,def.\ $\Leftrightarrow$ $\mathbf{Q}$ sym.\ und alle EW$(\mathbf{Q})>0$\\[0.05em]
    oder: alle Hauptunterdet.\,$>0$:\\[0.03em]
    \quad$q_{11}>0$,\;
    $\det\!\left[\begin{smallmatrix}q_{11}&q_{12}\\q_{21}&q_{22}\end{smallmatrix}\right]>0$,\;\ldots\\[0.08em]
    $\mathbf{Q}$ neg.\,def.\ $\Leftrightarrow$ $-\mathbf{Q}$ pos.\,def.\\[0.08em]
    $V=\mathbf{x}^T\!\mathbf{Q}\mathbf{x}$ pos.\,def.\ $\Leftrightarrow$ $\mathbf{Q}$ pos.\,def.} &
  \beispielblock{Q prüfen}{%
    \mathbf{Q}=\begin{bmatrix}2&1\\1&3\end{bmatrix}\\[0.1em]
    q_{11}=2>0\\
    \det(\mathbf{Q})=5>0\\
    \Rightarrow\text{pos.\,def.}} \\[0.3em]
  ...
  \formelblock{$\dot{V}$ berechnen}{\begin{aligned}
    ...
    &= \left(\frac{\partial V}{\partial\mathbf{x}}\right)^{\!T}
       \mathbf{f}(\mathbf{x})
  \end{aligned}} &
  \infoblock{\textbf{Einzugsgebiet:} Gesamtheit aller
    $\mathbf{x}(0)$, von denen aus
    $\mathbf{x}(t)\to\mathbf{x}_R$ für $t\to\infty$.\\[0.1em]
    \textbf{Nachweis:} Inneres einer
    Äqui-$V$-Linie $V(\mathbf{x})=c$, die
    vollständig in $\{\dot{V}(\mathbf{x})<0\}$ liegt.} &
