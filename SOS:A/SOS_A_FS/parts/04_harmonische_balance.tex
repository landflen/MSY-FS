...
\begin{formelreihe}
  \parbox[t]{\linewidth}{%
    ...
    in deren Nähe keine weitere\\
    period.\,Lösung existiert.\\[0.06em]
...
\begin{formelreihe}
  \parbox[t]{\linewidth}{%
    ...
    Bei statischer, symm.\,Kennlinie:\\
    Grundschw.\,von $a(t)$ hat\\
    gleiche Phasenlage wie $v(t)$\\[0.06em]
...
\noindent\footnotesize $v_0$ aus der HB ist die Amplitude am \textbf{NL-Eingang} (Argument von $K_N$). Übrige Signal\-amplituden durch \textbf{Zurück-/Vorwärtsrechnen} durch die Blöcke (jeweils Grundschwingung, Frequenz $\omega_0$):\par\vspace{0.15em}
\begin{rezept}
...
\noindent\footnotesize\textit{Statische, symm.\,Kennlinie \textbf{ohne Gedächtnis}: $K_N\in\mathbb{R}$ $\Rightarrow$ neg.\,inv.\,BF liegt auf neg.\,reeller Achse.\quad\textbf{Mit Gedächtnis} (Hysterese): $K_N\in\mathbb{C}$, Im$\!\left(-\tfrac{1}{K_N}\right)=-\tfrac{d\pi}{4h}=\mathrm{konst.}$}\par\vspace{0.12em}
...
\begin{formelreihe}
  ...
  \parbox[t]{\linewidth}{%
    ...
    \scriptsize
    Genau dann genau, wenn $G(s)$\\
    ausgeprägte Tiefpasswirkung hat\\
    (Oberwellen stark gedämpft)\\[0.04em]
    \textbf{Kein Schnittpunkt} $\not\Rightarrow$ kein GZ!\\[0.03em]
    Bsp.: PT1\,+\,Schalter\,m.\,Hyst.\\
    $\Rightarrow$ GZ existiert trotzdem\\[0.04em]
    Keine Aussage über Schwingung,\\
